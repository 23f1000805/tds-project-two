...
gemini_api = os.getenv("gemini_api_1")
...
client = OpenAI(api_key=API_KEY)  # Already initialized in your code
...
async def ping_chatgpt(question_text, relevant_context, max_tries=3):
    ...
    while tries < max_tries:
        try:
            print(f"OpenAI request attempt {tries + 1}/{max_tries}...")
            ...
            reply_text = response.choices[0].message.content.strip() if response.choices else None
            ...
            tries += 1
            last_exception = e
            print(f"[Error] Attempt {tries} failed: {e}")
            traceback.print_exc()
            await asyncio.sleep(min(2 ** tries, 10))  # exponential backoff

    # If all retries fail, return failure details
    return {
        "status": "error",
